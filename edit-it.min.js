...
$.fn.editit = function(event, options) {
  var $selector, defaults, onChange, settings;
  defaults = {
    url: "",
    data: {},
    done: function() {},
  ...
  onChange = function(input) {
    ...
    return $.post(settings.url, settings.data, settings.success).done(settings.done);
  };
  $(document).on(event, $selector, function() {
    ...
    input = $("<input type=\"text\" value=\"" + ($(this).html()) + "\" name=\"" + ($(this).data("name")) + "\">");
    $(this).html(input);
    input.focus().select();
    return input.on({
      focusout: function() {
